...
#: piece in it, and names average forty-odd bytes once escaped, so a long
#: enough list overruns the proxy's request-line buffer and comes back 414
#: from nginx with nothing from this app in it: an error the page cannot read
#: and the operator cannot find. Bounded here, an over-long list is a 400 that
#: says so, and the caller's job is to ask in batches.
MAX_FILENAMES = 200
